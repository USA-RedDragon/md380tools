...
void con_printc(char c) { con_addchar(c); }

static void con_putch(void* p, char c) { con_putc(c); }
...
static void con_draw1() {
    ...
    gfx_blockfill(
        0, MAX_Y - 9, MAX_X,
        MAX_Y); // fix by MAX_Y-9 instead of -10 overwrite of lowest pixel/line in bottom area of netmon screens
    // erase left 1 pixels
