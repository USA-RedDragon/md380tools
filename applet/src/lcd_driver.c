...
{
    while (nRepeats--) { // ST7735 datasheet V2.1 page 39, "8-bit data bus for 16-bit/pixel,
    ...
    while ((c = *cp++) != 0) {
        switch (c) {
            ...
            case '\t':   // horizontally CENTER the text in the rest of the line
                ...
                while ((c = *cp2++) >= 32) {
                    w += LCD_GetCharWidth(pContext->font,
                                          c); // suited for proportional fonts (which we don't have here yet?)
                }
