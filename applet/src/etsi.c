...
struct TAContext talkerAlias = {.src = 0, .length = 0};
...
void dump_full_lc(lc_t* lc) {
    ...
    PRINT("flco=%02x %s fid=%d svc=%d src=%d dst=%d\n", flco, get_flco_str(lc), fid, opts, get_adr(lc->src),
          get_adr(lc->dst));
    return;
}
...
void decode_ta(lc_t* lc) {
    ...
    if (flco == 4) {
        ...
        const uint8_t  table[] = {7, 15, 23, 31, 6, 13, 20, 27, 6, 13, 20, 27, 3, 6, 10, 13};
        uint8_t        length  = taContext.length;
        const uint8_t* row     = table + taContext.format * 4;
        taContext.blocks = 0b0001 | ((length > row[0]) << 1) | ((length > row[1]) << 2) | ((length > row[2]) << 3);
