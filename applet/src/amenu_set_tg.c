...
//---------------------------------------------------------------------------
void CheckTalkgroupAfterChannelSwitch(
    void) // [in] ad_hoc_tg_channel,ad_hoc_talkgroup; [out] contact.xyz
          ...
          // including the transition of channel_num = 0 -> channel_num from channel knob.
{
    ...
    {                     // Don't modify anything here. Tytera is just going to overwrite the "wanted" talkgroup !
               ...
               ad_hoc_tg_channel) { // When on THIS channel, should we be on the 'ad-hoc entered' talkgroup ?
        ...
        } else // switch back to the "wanted" talkgroup, set by user / alternative menu:
        {
            ...
            snprintfw(contact.name, 16, "%s %d*",
                      (ad_hoc_call_type == CONTACT_GROUP || ad_hoc_call_type == CONTACT_GROUP2 ? "TG" : "P"),
                      ad_hoc_talkgroup); // (trick from PR #708)
        }
    ...
    {      // e.g. operator switched to different channel.
        ...
        if (memcmp(channel_name, cn_override_group_prefix, 8) == 0) {
            ...
            snprintfw(contact.name, 16, "%s %d*", (contact.type == CONTACT_GROUP ? "TG" : "P"),
                      tgOverride); // (trick from PR #708)
        }
    }
