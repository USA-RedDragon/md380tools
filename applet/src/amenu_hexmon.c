...
//  the start address in the 'test / debug' menu (or whatever the final name will be) .
{
    ...
    while (dc.y < LCD_SCREEN_HEIGHT) {
        ...
        if ((addr >= HEXMON_DUMMY_ADDRESS_SPI_FLASH_START) &&
            (addr <= HEXMON_DUMMY_ADDRESS_SPI_FLASH_END)) { // Read from SPI flash (not directly accessable like RAM or
                                                            // internal Flash) :
        ...
        while (i < bytes_per_line) {
            if ((addr >= HEXMON_DUMMY_ADDRESS_SPI_FLASH_START) &&
                (addr <=
                 HEXMON_DUMMY_ADDRESS_SPI_FLASH_END)) { // Do NOT calculate a checksum when inspecting the SPI-flash !
                rd_value = b32Temp[i];
    ...
    switch (event) // what happened, why did the menu framework call us ?
    {
        ...
        case APPMENU_EVT_PAINT:                               // paint into the framebuffer here ?
            ...
            { // To minimize QRM from the display cable, only redraw the screen
                ...
                if ((addr >= HEXMON_DUMMY_ADDRESS_SPI_FLASH_START) &&
                    (addr <= HEXMON_DUMMY_ADDRESS_SPI_FLASH_END)) { // Do NOT calculate a checksum when inspecting the
                                                                    // SPI-flash !
                    checksum = HexMon_u32Checksum;
                } else { // only if NOT within the dummy-address-range for the SPI flash ...
                    checksum = 0xFFFF;
