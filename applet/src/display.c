...
char             eye_paltab[] = {0xd7, 0xd8, 0xd6, 0x00, 0x88, 0x8a, 0x85, 0x00, 0xe1, 0xe2, 0xe0, 0x00, 0xff, 0xff,
                                 0xff, 0x00, 0xae, 0xae, 0xaf, 0x00, 0x24, 0x4e, 0x8a, 0x00, 0x5d, 0x88, 0xbb, 0x00,
                                 0xd1, 0xd2, 0xd4, 0x00, 0xf4, 0xf4, 0xf4, 0x00, 0x3c, 0x66, 0x9f, 0x00, 0xdb, 0xe6,
                                 0xf3, 0x00, 0x48, 0x73, 0xaa, 0x00, 0xb6, 0xb8, 0xb4, 0x00, 0x5e, 0x6a, 0x77, 0x00};
char             eye_pix[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x10,
                              0x11, 0x10, 0x00, 0x00, 0x23, 0x41, 0x11, 0x31, 0x00, 0x01, 0x14, 0x55, 0x55, 0x61, 0x00,
                              0x21, 0x75, 0x88, 0x59, 0x94, 0x31, 0x3a, 0x85, 0x88, 0x56, 0x57, 0x73, 0x21, 0x86, 0x55,
                              0x5b, 0x67, 0x41, 0x13, 0x48, 0x66, 0x69, 0x71, 0xc1, 0x0c, 0x13, 0x47, 0x33, 0x11, 0x10,
...
int intCentibel(long ampli) {
    ...
    // 1 <= ampli < 32
    static const short fine[] = {-1,  0,   60,  95,  120, 140, 156, 169, 181, 191, 200, 208, 216, 223, 229, 235,
                                 243, 249, 253, 258, 262, 266, 270, 274, 278, 281, 285, 288, 291, 294, 297, 300};
...
#define STATECOUNTRY_MAX_LARGEFONT_CHARS 15
    ...
    if (global_addl_config.userscsv > 1 &&
        talkerAlias.length > 0) { // 2017-02-19 show Talker Alias depending on setup 0=CPS 1=DB 2=TA 3=TA & DB
        // TA or TA/DB mode
...
#define STATECOUNTRY_MAX_LARGEFONT_CHARS 14
    ...
    if (global_addl_config.mode_color == 1) {
        gfx_set_fg_color(0xffffff);
        gfx_set_bg_color(0xff4f32);
    }
    if (grp) {
...
void draw_alt_statusline() {
    ...
    if (src == 0) {
        ...
        if (global_addl_config.datef == 6 &&
            talkerAlias.length > 0) // 2017-02-18 show talker alias in status if rcvd valid
        {
            ...
            if (usr_find_by_dmrid(&usr, src) == 0) {
                ...
                if (usr_find_by_dmrid(&dst, rst_dst) != 0) {
                    ...
                    gfx_printf_pos2(RX_POPUP_X_START, 96, 157, "LH:%s %s>%s %c", usr.callsign, firstname, dst.callsign,
                                    mode);
                } else {
                    firstname = get_firstname(&usr, firstname_buf, FIRSTNAME_BUFSIZE);
                    gfx_printf_pos2(RX_POPUP_X_START, 96, 157, "LH:%s %s>%d %c", usr.callsign, firstname, rst_dst,
                                    mode);
                }
            }
        }
...
void draw_adhoc_statusline() {
    //	int x = RX_POPUP_X_START + 36;							// 36=standard position aligned
    //with channel info
    ...
    if (!fIsDigital) { // DMR channel active
        ...
        int tgNum =
            (ad_hoc_tg_channel
                 ? ad_hoc_talkgroup
                 : (((int)contact.id_h << 16) | ((int)contact.id_m << 8) | (int)contact.id_l)); // current talkgroup
        ...
        if (global_addl_config.mode_stat != 0) {
            ...
            if (global_addl_config.mode_stat != 3) { // if MODE/CC compact display set in config
                ...
                gfx_printf_pos2(x, top_y, 120, "%s%d|%d|%s%s%d%s%s", top_status, ch_cc, ch_ts,
                                (ad_hoc_tg_channel ? "A" : ""),
                                (callType == CONTACT_GROUP || callType == CONTACT_GROUP2 ? "" : "P"), tgNum, tg_fill,
                                mic_gain_stat);
            }
            gfx_set_fg_color(0x000000);
            gfx_set_bg_color(0xff8032);
        ...
        if (global_addl_config.chan_stat != 0) {
            if (usr_find_by_dmrid(&usr, tgNum) == 0) {
                if (global_addl_config.chan_color == 1) {
                    gfx_set_fg_color(0x261162);
                    gfx_set_bg_color(0xff9f32);
                }

                if (global_addl_config.chan_stat == 1) {     // show TS / TG / CTS / DCS status
                    if (global_addl_config.mode_stat != 3) { // if MODE/CC compact display set in config
                        gfx_printf_pos2(
                            x, y, 120, "%s%d %s%s:%d          ", bot_status, ch_ts, (ad_hoc_tg_channel ? "Ad" : ""),
                            (callType == CONTACT_GROUP || callType == CONTACT_GROUP2 ? "TG" : "Priv"), tgNum);
                    } else {
                        if (global_addl_config.chan_stat !=
                            4) { // top=compact - bottom not rx/tx, so show rx, or if 3 = tx
                            gfx_printf_pos2(x, y, 120, "%s:%s MHz   ",
                                            (global_addl_config.chan_stat == 3 ? "TX" : "RX"),
                                            (global_addl_config.chan_stat == 3 ? freq_tx : freq_rx));
                        } else {
                    ...
                    if (global_addl_config.chan_stat != 4) {
                        gfx_printf_pos2(x, y, 120, "%s:%s MHz   ", (global_addl_config.chan_stat == 3 ? "TX" : "RX"),
                                        (global_addl_config.chan_stat == 3 ? freq_tx : freq_rx));
                    } else {
                        if (fIsSimplex) {
                            gfx_printf_pos2(x, y - 3, 120, "-- Simplex -- ");
                ...
                if (global_addl_config.chan_stat == 1) {
                    gfx_printf_pos2(
                        x, y, 120, "%s%d %s%s:%s          ", bot_status, ch_ts, (ad_hoc_tg_channel ? "Ad" : ""),
                        (callType == CONTACT_GROUP || callType == CONTACT_GROUP2 ? "TG" : "Priv"), usr.callsign);
                } else {
                    if (global_addl_config.chan_stat < 4) {
                        gfx_printf_pos2(x, y, 120, "%s:%s MHz   ", (global_addl_config.chan_stat == 3 ? "TX" : "RX"),
                                        (global_addl_config.chan_stat == 3 ? freq_tx : freq_rx));
        ...
        if (global_addl_config.mode_stat != 3) { // if MODE/CC compact display set in config
            ...
            if (*ch_tone_type != 'N') { // if MODE/CC compact display set in config (and CTCSS not set to None)
                if (fIsCTSvalid) {
                    ...
                    strcat(top_status, ":");
                    strcat(top_status,
                           current_channel_info_E.EncTone.text); // add DCS/CTS tone to topstatus in compact mode
                }
            } else {
                strcat(top_status, "  ");
            }
