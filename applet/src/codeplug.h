...
extern wchar_t zone_name_2[32]; // 64 bytes @ 0x2001e218 in D13.020
                                // The first half of zone_name_2 @ 0x2001e218 contained the same as zone_name @
                                // 0x2001cddc . zone_name_2 is the destination buffer when reading from SPI-lash - see
                                // 0x08022d74 . See also : "how to switch to a different zone" in amenu_codeplug.c .
#endif
...
inline static int current_TG() { return (((int)contact.id_h << 16) | ((int)contact.id_m << 8) | (int)contact.id_l); }
#endif
