...
void lcd_background_led(int on) { // 2017-01-07 : Never called / no effect ? The backlight seems to be controlled "by
                                  // Tytera only" (backlight_timer).
...
void print_ant_sym_hook(char* bmp, int x, int y) {
    ...
    gfx_drawbmp(bmp, x, y);
    draw_eye_opt();
    // if ( global_addl_config.userscsv > 1 && talkerAlias.length > 0 )                              // 2017-02-19 show
    // talker alias if option selected and rcvd valid alias
    //       {
    //       draw_rx_screen(0xff8032);                                                               // 2017-02-18
    //       redraw userinfo when valid talker alias rcvd
    //       }
#endif
}
