/*! \file irq_handlers.c
  ...
  Please don't poison this sourcecode with TAB characters .
  Please don't poison this sourcecode with TAB characters .  Thank you .

  Details may still be at www.qsl.net/dl4yhf/RT3/md380_fw.html#dimmed_light .
  Latest modifications:
    2017-09-03, DL4YHF : Removed tabs and fixed broken formatting / indentation.
          Hard a hard time to merge modifications from the DL4YHF branch
          (which had a pull-request pending for merge since 2017-06-21)
          with the master git repo (travisgoodspeed/md380tools).
    2017-??-??, DL2MF  : Added PollKeysForScroll() [for TYTERA's menu]
          and support for MD-446 keybd.
...
volatile uint32_t IRQ_dwSysTicksAtBoot =
    0; // snapshot of IRQ_dwSysTickCounter when 'essential' boot_flags were first set
...
#define MORSE_GEN_START                                                                                                \
    1                               // request to start output
...
#define MORSE_GEN_PASSIVE_NOT_MUTED                                                                                    \
    11                                    // "should be passive but couldn't turn off the PA yet"
...
uint8_t Morse_table[] = { // Morse code table, without mutated vowels, diacritics & Co
                          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
#if (CONFIG_DIMMED_LIGHT)
static void InitDimming(void) { // Called from SysTick_Handler() when Tytera's firmware(!) has initialized
    ...
    // There are FOUR bits per pin in AFR[0..1], thus PC6 is in AFR[0] bits 27..24. USART6_TX = "AF8" (STM32F405/7 DS
    // page 62).
    ...
    pUSART->CR1 /*pg 1014*/ = (1 /*enable*/ << 13) | (1 /*9bit-mode*/ << 12) |
                              (1 /*TE*/ << 3); // 1 startbit, 9 data bits, no parity, no interrupts, "OVER8"=0
...
#if (CONFIG_MORSE_OUTPUT)
    if (morse_generator.u8State != MORSE_GEN_PASSIVE) { // "Morse generator is active so please DON'T scan channels now"
...
int MorseGen_AppendChar(char c) {
    ...
    if (pMorseGen->u8FifoHead != pMorseGen->u8FifoTail) {
        if (pMorseGen->u8State == MORSE_GEN_PASSIVE || pMorseGen->u8State == MORSE_GEN_PASSIVE_NOT_MUTED) {
...
// via hardware (simple PWM, no CPU-hogging 'wavetable').
{
    ...
    if (pMorseGen->u16Timer > 0) // state timer not expired yet ?
    {
        ...
        if ((pMorseGen->u8State == MORSE_GEN_SENDING_TONE) || (pMorseGen->u8State == MORSE_GEN_SENDING_GAP) ||
            ...
            if ((!IS_AUDIO_AMP_ON) || (!IS_SPKR_SWITCH_ON)) { // Gotcha.. the original firmware interfeared :)
                ...
                if (global_addl_config.narrator_mode & NARRATOR_MODE_TEST) { // show when something 'interfered':
                    if (!IS_RED_LED_ON) {
    ...
    } else // timer expired, so what's up next (state transition) ?
    {
        ...
        switch (pMorseGen->u8State) {
            ...
            case MORSE_GEN_START_CHAR_TX:                             // start transmission of next char
                if (pMorseGen->u8FifoHead != pMorseGen->u8FifoTail) { // there's more to send...
                    MorseGen_BeginToSendChar(pMorseGen, pMorseGen->u8Fifo[pMorseGen->u8FifoTail++]);
            ...
            case MORSE_GEN_END_OF_MESSAGE: // all characters sent, 'back to normal'
                ...
                if (IsRxAudioMuted()) { // guess it's ok to turn the audio-PA off now:
...
//---------------------------------------------------------------------------
char KeyRowColToASCII(uint16_t kb_row_col) { // Converts a hardware-specific keyboard code into a character (ASCII) .
    ...
    if (global_addl_config.keyb_mode == 2) { // support for MD-446 keyb layout
        ...
        {
            case 0x000A:
                return 'B'; // 'back' aka 'red button'
            case 0x0012:
                return 'U'; // cursor up
            case 0x0022:
                return 'M'; // Green 'Menu' key
            ...
            case 0x002A:
                return 'X'; // eXit all menus and sub-menus
            default:
                return 0;
        }
    } else // not MD-446 but MD380, MD390, RT3, RT8, ..(?)
    {
        ...
        {
            case 0x000A:
                return 'M'; // Green 'Menu' key (which usually opens TYTERA's menu)
            case 0x000C:
                return '1';
            case 0x0012:
                return 'U'; // cursor up
...
int KeyRowColToVal(uint16_t kb_row_col) {
    if (global_addl_config.keyb_mode == 2) { // support for MD-446 keyb layout
        ...
        {
            case 0x000A:
                return 13; // 'back' aka 'red button'
            case 0x000C:
                return 4; // P2
            case 0x0012:
                return 11; // cursor up
            case 0x0014:
                return 14; // cursor down
            case 0x0022:
                return 30; // Green 'Menu' key
            case 0x0024:
                return 7; // P1
                          // kb_row_col_pressed also supports a few COMBINATIONS:
                          //   MENU+BACK (simultaneously pressed) : 0x040A
            case 0x002A:
                return 55; // eXit all menus and sub-menus
            default:
                return 0;
        }
    } else {
        ...
        {
            case 0x000A:
                return 30; // Green 'Menu' key (which usually opens TYTERA's menu)
            case 0x000C:
                return 1;
            case 0x0012:
                return 11; // cursor up
...
// like SysTick(). Call those functions from a suitable task, not from here.
{
    ...
    if ((gui_opmode2 == OPM2_MENU) && (!Menu_IsVisible())) { // keyboard focus currently on Tytera's 'green key' menu
...
// Almost a duplicate of PollKeys(), but for auto-repeat in TYTERA's menu.
// Formatting destroyed by TAB characters with a non-standard, unknown TAB setting. Yucc.
{
    static uint16_t prev_key;
    ...
    if (gui_opmode2 == OPM2_MENU) { // keyboard focus currently on Tytera's 'green' menu
        // -> ignore kb_row_col_pressed until the key was released
...
// because we also call the original handler from here.
{
    ...
    if (!(boot_flags &
          BOOT_FLAG_INIT_BACKLIGHT)) { // Did the original firmware turn on the backlight ?
                                       // First of all, did it turn on the peripheral clocks for GPIO A..E ?
#define RCC_AHB1ENR_GPIO_A_to_E                                                                                        \
    (RCC_AHB1ENR_GPIOAEN | RCC_AHB1ENR_GPIOBEN | RCC_AHB1ENR_GPIOCEN | RCC_AHB1ENR_GPIODEN | RCC_AHB1ENR_GPIOEEN)
        if ((RCC->AHB1ENR & RCC_AHB1ENR_GPIO_A_to_E) ==
            RCC_AHB1ENR_GPIO_A_to_E) { // Did the firmware configure backlight and RX,TX-LEDs as output ?
            if (((GPIOC->MODER & (3 << (PINPOS_C_BL * 2))) == (1 << (PINPOS_C_BL * 2))) &&
                ((GPIOE->MODER & (3 << (PINPOS_E_TX * 2))) == (1 << (PINPOS_E_TX * 2))) &&
                ((GPIOE->MODER & (3 << (PINPOS_E_RX * 2))) ==
                 (1 << (PINPOS_E_RX * 2)))) { // Did the firmware turn the backlight on (for the 1st time) ?
    ...
    if ((boot_flags & (BOOT_FLAG_INIT_BACKLIGHT | BOOT_FLAG_LOADED_CONFIG | BOOT_FLAG_DREW_STATUSLINE)) !=
        (BOOT_FLAG_INIT_BACKLIGHT | BOOT_FLAG_LOADED_CONFIG |
         BOOT_FLAG_DREW_STATUSLINE)) { // As long as the original firmware is still "booting",
    ...
    if (boot_flags &
        BOOT_FLAG_OPEN_FOR_BUSINESS) { // 2017-05-14 : Removed the brightness 'ramp-up' test.
                                       //   The 9 intensity levels can now be tested in app_menu.c in inc/dec edit mode.
#if (CONFIG_DIMMED_LIGHT)              // Support dimmed backlight ?
        if ((backlight_timer > 0) ||
            (md380_radio_config.backlight_time ==
             0)) {           // If the backlight time is ZERO, use the 'radio-active' setting ("backlight level HI")
        ...
        {
            GPIOC->MODER = (GPIOC->MODER & ~(3 << (6 /*pin*/ * 2))) |
                           (2 << (6 /*pin*/ * 2)); // PC6 now configured as UART6_TX again
            switch (intensity)                     // which UART-data to send as PWM ?
            {
                case 1:
                    USART6->DR = 0x00; // low intensity: backlight only driven by stopbit.
                    // Theoretic duty cycle (DC): 100% * 0.5 / (1+9+0.5) = 4.76 %
                    break;
                case 2:
                    USART6->DR = 0x100;
                    break; // DC = 100% * 1.5 / 10.5 = 14.3 %
                    // A UART sends the LSBit first. For the lowest possible RFI,
                    // we only want ONE PULSE in each cycle. Thus 0x100, not 0x001 .
                    break;
                case 3:
                    USART6->DR = 0x180;
                    break; // DC = 100% * 2.5 / 10.5 = 23.8 %
                case 4:
                    USART6->DR = 0x1C0;
                    break; // DC = 100% * 3.5 / 10.5 = 33.3 %
                case 5:
                    USART6->DR = 0x1E0;
                    break; // DC = 100% * 4.5 / 10.5 = 42.3 %
                case 6:
                    USART6->DR = 0x1F0;
                    break; // DC = 100% * 5.5 / 10.5 = 52.4 %
                case 7:
                    USART6->DR = 0x1F8;
                    break; // DC = 100% * 6.5 / 10.5 = 61.9 %
                case 8:
                    USART6->DR = 0x1FC;
                    break; // DC = 100% * 7.5 / 10.5 = 71.4 %
                default:
                    break; // max possible brightness, 100% duty cycle,
        ...
        if (oldSysTickCounter > 3000) {          // Some seconds after power-on, begin to poll analog inputs...
        ...
        if (morse_generator.u8State != MORSE_GEN_PASSIVE) { // Only spend time on this when active !
