...
const static wchar_t* wt_bl_intensity[NUM_BACKLIGHT_INTENSITIES] = {
    L"0 (off)", L"1 (lowest)", L"2", L"3 (medium)", L"4", L"5", L"6", L"7", L"8", L"9 (bright)"};
...
const static uint8_t button_functions[] = {
    0x00,
    0x01,
    0x02,
    0x03,
...
#define MKTHUMB(adr)                                                                                                   \
    (printf(((int)adr) & 1 ? "" : "Warning, 0x%08x function pointer is even.\n", adr), (void(*))(((uint32_t)adr) | 0x1))

void create_menu_entry_rev(int menuid, const wchar_t* label, void (*green_key)(), void (*red_key)(), int e, int f,
                           int item_count) {
...
// void md380_create_menu_entry(int menuid, const wchar_t * label , void * green_key, void  * red_key, int e, int f ,int
// enabled) { #ifdef DEBUG
...
menu_t* get_menu_stackpoi() { return &md380_menu_memory[md380_menu_depth + 1]; }
...
void mn_create_single_timed_ack(const wchar_t* title, const wchar_t* label) {
    ...
    md380_create_menu_entry(md380_menu_id, label, MKTHUMB(md380_menu_entry_back), MKTHUMB(md380_menu_entry_back), 6, 2,
                            1);
}
...
void mn_submenu_add(const wchar_t* label, void (*func)()) {
    ...
    md380_create_menu_entry(md380_menu_id + menu_mem->numberof_menu_entries, label, func,
                            MKTHUMB(md380_menu_entry_back), 0x8b, 0, 1);
...
void mn_submenu_add_98_enabled(const wchar_t* label, void (*func)(), int enabled) {
    ...
    md380_create_menu_entry(md380_menu_id + menu_mem->numberof_menu_entries, label, func,
                            MKTHUMB(md380_menu_entry_back), 0x98, 0, enabled);

    menu_mem->numberof_menu_entries++;
}

void mn_submenu_add_98(const wchar_t* label, void (*func)()) { mn_submenu_add_98_enabled(label, func, 1); }
...
void mn_submenu_add_8a(const wchar_t* label, void (*func)(), int enabled) {
    ...
    md380_create_menu_entry(md380_menu_id + menu_mem->numberof_menu_entries, label, func,
                            MKTHUMB(md380_menu_entry_back), 0x8a, 0, enabled);
...
uint32_t get_effective_dmrid() { return global_addl_config.dmrid; }
...
void mn_backlight_always() { mn_backlight_set(0, wt_blalways); }

void mn_backlight_5sec() { mn_backlight_set(1, wt_bl5); }

void mn_backlight_10sec() { mn_backlight_set(2, wt_bl10); }

void mn_backlight_15sec() { mn_backlight_set(3, wt_bl15); }

void mn_backlight_30sec() { mn_backlight_set(6, wt_bl30); }

void mn_backlight_60sec() { mn_backlight_set(12, wt_bl60); }
...
void mn_backlight(void) // menu for the backlight-TIME (longer than Tytera's, but sets the same parameter)
{
    ...
    {                                          // (fixes the selection of the current backlight-time in the menu)
        ...
        case 3 /* times 5sec */:
            ...
            break;
        case 6 /* times 5sec */:
            md380_menu_entry_selected = 4;
            break;
        case 12 /* times 5sec */:
            md380_menu_entry_selected = 5;
            break;
        default /* always  */:
            md380_menu_entry_selected = 0;
            break;
...
void select_sidebutton_function_screen(void) {
    ...
    switch (button_selected) {
        case 0:
            md380_menu_entry_selected =
                index_of(top_side_button_pressed_function, (uint8_t*)button_functions, sizeof(button_functions));
            // cast to (uint8_t*) to eliminate several 'const' warnings
            mn_submenu_init(wt_button_top_press);
            break;
        case 1:
            md380_menu_entry_selected =
                index_of(bottom_side_button_pressed_function, (uint8_t*)button_functions, sizeof(button_functions));
            mn_submenu_init(wt_button_bot_press);
            break;
        case 2:
            md380_menu_entry_selected =
                index_of(top_side_button_held_function, (uint8_t*)button_functions, sizeof(button_functions));
            mn_submenu_init(wt_button_top_held);
            break;
        case 3:
            md380_menu_entry_selected =
                index_of(bottom_side_button_held_function, (uint8_t*)button_functions, sizeof(button_functions));
...
void create_menu_entry_backlight_screen(void) {
    ...
    mn_submenu_add_98(wt_bl_intensity_lo /*item text*/,
                      mn_backlight_lo /*menu handler*/); // backlight intensity "low" (used when idle)
    mn_submenu_add_98(wt_bl_intensity_hi /*item text*/,
                      mn_backlight_hi /*menu handler*/); // backlight intensity "high" (used when active)
#endif
...
void mn_cw_volume(void) { // Also here, a simple DECIMAL input field would be nice to have...
...
void mn_cw_speed(void) {
    ...
    switch (global_addl_config.cw_speed_WPM) {
        ...
        case 40:
            md380_menu_entry_selected = 5;
            break; // for complete nuts
        default:
            md380_menu_entry_selected = 2;
            break; // meaningful default ?
...
#ifdef CONFIG_MENU
    md380_create_menu_entry(md380_menu_id, md380_menu_edit_buf, MKTHUMB(md380_menu_entry_back),
                            MKTHUMB(md380_menu_entry_back), 6, 1, 1);
#endif
}
...
#ifdef CONFIG_MENU
    md380_create_menu_entry(md380_menu_id, wt_edit, MKTHUMB(create_menu_entry_edit_screen_store),
                            MKTHUMB(md380_menu_numerical_input), 0x81, 0, 1);
#endif
}
...
#ifdef CONFIG_MENU
    md380_create_menu_entry(md380_menu_id, wt_edit_dmr_id, MKTHUMB(create_menu_entry_edit_dmr_id_screen_store),
                            MKTHUMB(md380_menu_numerical_input), 0x81, 0, 1);
#endif
}
...
#ifdef CONFIG_MENU
    md380_create_menu_entry(md380_menu_id, wt_set_tg_id, MKTHUMB(create_menu_entry_set_tg_screen_store),
                            MKTHUMB(md380_menu_numerical_input), 0x81, 0, 1);
#endif
#endif // D13_020, S13_020, ..(?)
}
...
#ifdef CONFIG_MENU
    md380_create_menu_entry(8, md380_wt_programradio, MKTHUMB(md380_menu_entry_programradio),
                            MKTHUMB(md380_menu_entry_back), 0x8a, 0, enabled);

#ifdef FW_D13_020
    md380_create_menu_entry(11, wt_main_menu, MKTHUMB(create_main_md380tools_screen), MKTHUMB(md380_menu_entry_back),
                            0x8a, 0, 1);
#else
    if (menu_mem->numberof_menu_entries == 6) { // d13.020 has hidden gps entrys on this menu
        md380_create_menu_entry(11, wt_main_menu, MKTHUMB(create_main_md380tools_screen),
                                MKTHUMB(md380_menu_entry_back), 0x8a, 0, 1);
    } else {
        md380_create_menu_entry(9, wt_main_menu, MKTHUMB(create_main_md380tools_screen), MKTHUMB(md380_menu_entry_back),
                                0x8a, 0, 1);
    }
#endif
...
#ifdef CONFIG_MENU
    md380_create_menu_entry(md380_menu_id + (menu_mem->numberof_menu_entries - 1), wt_set_contact, MKTHUMB(set_contact),
                            MKTHUMB(md380_menu_entry_back), 0x8a, 0, 1);
    // Original Item
    md380_create_menu_entry(md380_menu_id, md380_wt_view_number, MKTHUMB(md380_menu_entry_view_number),
                            MKTHUMB(md380_menu_entry_back), 0x8a, 0, 1);
...
void create_menu_contact_details_private_hook(void) {
    ...
    md380_create_menu_entry(md380_menu_id + (menu_mem->numberof_menu_entries - 1), wt_set_contact, MKTHUMB(set_contact),
                            MKTHUMB(md380_menu_entry_back), 0x8a, 0, 1);
    // Original Item
    md380_create_menu_entry(md380_menu_id, md380_wt_call_alert, MKTHUMB(md380_menu_entry_call_alert),
                            MKTHUMB(md380_menu_entry_back), 0x8a, 0, 1);
