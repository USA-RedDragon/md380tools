...
const am_stringtable_t am_stringtab_onoff[] = {
    {0, "Off"}, {1, "On"}, {0, NULL} // <- marks the end of a string table
};
#endif
...
uint8_t getCC(channel_t* ch) { return ((((uint8_t*)ch)[1] & 0xF0) >> 4) & 0xF; }
void    setCC(channel_t* ch, uint8_t cc) { ((uint8_t*)ch)[1] = (((uint8_t*)ch)[1] & 0x0F) | ((cc << 4) & 0xF0); }
/*
...
void setType(channel_t* ch, uint8_t type) { ((uint8_t*)ch)[0] = (((uint8_t*)ch)[1] & (~0x01)) | (type ? 0x01 : 0); }

uint8_t getAutoScan(channel_t* ch) { return ((ch->settings[0] & 0x10) != 0 ? 1 : 0); }
void    setAutoScan(channel_t* ch, uint8_t val) { ch->settings[0] = (ch->settings[0] & (~0x10)) | (val ? 0x10 : 0); }

uint8_t getSlot(channel_t* ch) { return ((((uint8_t*)ch)[1] & 0x04) != 0 ? 1 : 2); }
...
int readFrequency(channel_t* ch, frequency_t* freq, char fRx) {
    ...
    sprintf(freq->text, "%d%d%d.%d%d%d%d%d\0\0", freq->digits[7], freq->digits[6], freq->digits[5], freq->digits[4],
            freq->digits[3], freq->digits[2], freq->digits[1], freq->digits[0]);
    return 0;
}
...
int readTone(channel_t* chan, tone_t* tone, char fEnc) {
    ...
    if (tone->fType == 'C') {
        tone->freq = ((float)tone->digits[0] * 100.0f) + ((float)tone->digits[1] * 10.0f) +
                     ((float)tone->digits[2] * 1.0f) + ((float)tone->digits[3] * 0.1f);
    } else {
        tone->freq = 0.0f;
    }
