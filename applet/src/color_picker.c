...
static void ColorPicker_DrawBargraph(int y1, int y2, int index, int value) {
    ...
    switch (index) {
        case 0:
            wColor = LCD_COLOR_RED;
            break;
        case 1:
            wColor = LCD_COLOR_GREEN;
            break;
        default:
            wColor = LCD_COLOR_BLUE;
            break;
    }
...
//---------------------------------------------------------------------------
static void
ColorPicker_Draw(app_menu_t*  pMenu,
                 menu_item_t* pItem) { // For simplicity, the "colour picker" always redraws its screen COMPLETELY.
    int           x, y, y2, i, sel;
    char*         cp;
    ...
    color[0] =
        LCD_GetGoodContrastTextColor(color[1]); // black or white text,
...
//---------------------------------------------------------------------------
static void ColorPicker_IncDec(app_menu_t* pMenu, int delta) { // Called when modifying one of the 3 colour components
    rgb_quad_t rgb;
    int        n, iField = pMenu->dialog_field_index;
    rgb.u32 = LCD_NativeColorToRGB((uint16_t)pMenu->iEditValue);
    if (iField >= 0 && iField <= 2) { // RGB components range from 0 to 255 but the LCD suports BRG565 only.
...
const color_scheme_t color_schemes[] = { // name              fg_color bg_color sel_fg  sel_bg  edit_fg  edit_bg
...
//
{
    ...
    if (pSL->focused_item >= 0 &&
        pSL->focused_item < pSL->num_items) { // use the currently focused colour scheme to draw the title
...
// Callback function, invoked from the "app menu" framework to select a colour palette.
{
    ...
    if (event == APPMENU_EVT_ENTER) // pressed ENTER (just entered this screen) ?
    {
        ...
        for (i = 0; i < pSL->num_items; ++i) {
            pScheme = &color_schemes[i];
            if ((pCfg->fg_color == pScheme->fg_color) && (pCfg->bg_color == pScheme->bg_color) &&
                (pCfg->sel_fg_color == pScheme->sel_fg_color) && (pCfg->sel_bg_color == pScheme->sel_bg_color) &&
                (pCfg->edit_fg_color == pScheme->edit_fg_color) && (pCfg->edit_bg_color == pScheme->edit_bg_color)) {
        ...
        switch ((char)param) // here: message parameter = keyboard code (ASCII)
        {
            case 'M': // green "Menu" key : kind of ENTER. Here, "apply & return" .
                if (pSL->focused_item >= 0 && pSL->focused_item < (sizeof(color_schemes) / sizeof(color_schemes[0]))) {
                    ...
                    pMenu->save_on_exit =
                        TRUE; // don't save in SPI-flash here yet, but when returning from the app-menu
                }
                return AM_RESULT_EXIT_AND_RELEASE_SCREEN;
