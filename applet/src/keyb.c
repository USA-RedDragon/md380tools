...
inline int get_main_mode() { return gui_opmode1 & 0x7F; }
...
void evaluate_sidekey(int button_function) // This is where new functions for side buttons can be added
{
    switch (button_function) {    // We will start at 0x50 to avoid conflicting with any added functions by Tytera.
        ...
        case 0x54: // toggle display
            ...
            switch (global_addl_config
                        .chan_stat) { // We are using 0x54 to avoid conflicting with any added functions by Tytera.
                case 1:
...
void kb_handler_hook() {
    ...
    if (global_addl_config.keyb_mode == 2) { // keyb layout = MD-446 (select in setup)
        ...
        switch (kc) {
            case 10:
                if (kb_row_col_pressed ==
                    0x000A) // the MD-446 layout is something weird, so it needs some rewrites of keycodes!!!
                {
                    kb_keycode =
                        kc_redback;  // if the RedKey (key-10) was pressed, we need rewrite to RedKey function (kc=13)
                    kc = kc_redback; // RedKey - NEVER change this!!
                } else {
                    kb_keycode =
                        kc_greenmenu;  // if we got kc=10 not from 0x000A, this was rewrite result of GreenKey (key-12)
            ...
            case 3:
                if (!is_netmon_visible() || kc_lastmode == 0x000C) {
                    kc = kc_netmon4; // define your preferred P1 function: 0=Netmon6 2=Netmon5 4=Netmon4 8=Netmon1
                                     // 9=Netmon2 15=Netmon3
            ...
            case 1:
                if (!is_netmon_visible() || kc_lastmode == 0x0024) {
                    kc = kc_netmon1; // define your preferred P1 function: 0=Netmon6 2=Netmon5 4=Netmon4 8=Netmon1
                                     // 9=Netmon2 15=Netmon3
                    kc_lastmode = kb_row_col_pressed; // remember current mode for P1/P2 toggle keys
                } else {
                    kc = kc_netmon_off; // exit from Netmon screens
...
//   and maybe for auto-repeat in the original menu.
{
    ...
    switch (ascii) {
        ...
        case 'D':
            return KC_DOWN;
        case '*':
            return KC_STAR; // ex: KC_ASTERISK (?)
        case '#':
            return KC_HASH; // ex: KC_OCTOTHORPE (?)
        default:
            return KC_NO_VALID_KEY; // bleah.. cannot use 0x00 for this
    }
...
//       kb_keycode, kb_keypressed : set here to operate TYTERA's menu remotely .
{
    ...
    if (key_down_flag) { // key has just been PRESSED (on the remote keyboard) :
