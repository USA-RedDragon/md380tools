...
void print_hdr() { con_printf("hdr: %d:%d:%d\n", rst_hdr_src, rst_hdr_dst, rst_hdr_sap); }

void print_vce() { con_printf("vce: %d:%d %d\n", rst_src, rst_dst, rst_grp); }
...
void netmon1_update() {
    ...
    {
        con_printf("re:%02x be:%02x e3:%02x e4:%02x\ne5:%02x ", last_radio_event, last_event2, last_event3, last_event4,
                   last_event5);
    }
    print_smeter();
#if defined(FW_D13_020) || defined(FW_S13_020)
    {
        ...
        con_printf("st: %2x %2x %2x %2x\n", radio_status_1.m0, radio_status_1.m1, radio_status_1.m2, radio_status_1.m3);
    }
#endif
#ifdef FW_D13_020
    {
...
void print_bcd(uint8_t bcd) { con_printf("%d%d", (bcd >> 4) & 0xf, bcd & 0xf); }
...
void netmon3_update() { syslog_draw_poll(); }
...
void netmon5_update() {
    ...
    if (rst_voice_active) {
        ...
        if ((src != 0) && (rx_new == 1)) {
            ...
            if ((wcscmp(sh_last_channel, curr_channel) != 0) ||
                (sl_cnt >= 9)) { // compare channel_name with last_channel from latest rx
            ...
            if (global_addl_config.devmode_level >= 2) {
                PRINT("nm5> cc:%d ts1:%d ts2:%d lh:%d\n", cc, ts1, ts2, lh_ts);
            }
...
// using the alternative LCD driver (much faster than Tytera's).
{
    ...
    if (pMenu->redraw || (ReadStopwatch_ms(&stopwatch_for_update) >
                          1000 /*ms*/)) { // Draw the console into the framebuffer, using the alternative LCD
