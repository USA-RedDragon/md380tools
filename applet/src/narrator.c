                   ...
                   //
{
    ...
    if ((pNarrator->to_do != 0) && (ReadStopwatch_ms(&pNarrator->stopwatch) > 200 /*ms*/)) {
        ...
        {                                    // (don't wait for the buffer to run empty, avoid gaps,
            ...
            if (pNarrator->to_do & (NARRATOR_REPORT_CHANNEL_NAME |
                                    NARRATOR_REPORT_CHANNEL_NUMBER)) { // report the current channel (highest prio)
            ...
            if (pNarrator->to_do & NARRATOR_REPORT_ZONE) { // next lower priority, etc...
            ...
            if (pNarrator->to_do & NARRATOR_READ_CONSOLE) { // Such a 'long story' doesn't fit in the Morse TX buffer,
...
#endif                                            // CONFIG_APP_MENU ?
        if (pNarrator->focused_item_index >= 0) { // guess we're in the menu so start talking about it :
...
static void report_menu_item(void) {
    ...
    {
        Menu_ReportItemInMorseCode(AMENU_MORSE_REQUEST_ITEM_TEXT | AMENU_MORSE_REQUEST_ITEM_VALUE);
    } else
#endif // CONFIG_APP_MENU ?
...
//---------------------------------------------------------------------------
static void start_reading_console(void) { // gadget, useful feature, or an alternative 'Morse trainer' ?
    T_Narrator* pNarrator = &Narrator;
...
#if defined(FW_D13_020) || defined(FW_S13_020)
            itemIndex = *((uint8_t*)0x20004CC2); // "competing" menu-item-index in D13.020, BEFORE entering MD380Tools
                                                 // menu (bizarre...)
...
wchar_t* get_menu_title(void) // may return NULL when not in a menu !
{
    ...
    if (pMenu) { // In the MAIN MENU (simply titled "menu"),
                                           ...
                                           //                  -1 to retrieve only the CURRENTLY SELECTED item's text.
{
    ...
    if (pMenu) {
        ...
        if (itemIndex >= 0 && itemIndex < nItems) { // ex: return pMenu->entries[itemIndex].label;
