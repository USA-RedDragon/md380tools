    ...
    */
    ...
    {                                      // .. via 'Down'-key at power on ?
        LOGB("t=%d:USBup: l=%d,b=%d\n",    // visible in what used to be Netmon3 (2017-05-21)
             (int)IRQ_dwSysTickCounter, (int)length, (int)blockadr);
    }
#endif // DEBUG ?
...
// SPI-FLASH commands
            ...
            case TDFU_SPIFLASHREAD:
                ...
                *md380_dfu_target_adr = dmesg_tx_buf;
                ...
                md380_spiflash_read(dmesg_tx_buf, adr, DMESG_SIZE);
                break;
            ...
            case TDFU_SPIFLASHSECURITYREGREAD:
                ...
                *md380_dfu_target_adr = dmesg_tx_buf;
                ...
                md380_spiflash_security_registers_read(dmesg_tx_buf, 0, 3 * 256);
                break;
            ...
            case TDFU_PRINT: // 0x80, u8 x, u8 y, u8 str[].
                drawtext((wchar_t*)(md380_packet + 3), md380_packet[1], md380_packet[2]);
                break;
            ...
            case TDFU_READ_FRAMEBUFFER_24BPP: // read a tile with 24-bit RGB from the framebuffer.
                ...
                *md380_dfu_target_adr = dmesg_tx_buf;     // dmesg_tx_buf = (char*)DMESG_START = 0x2001F700
                ...
                if (LCD_busy) { // Don't "interrupt" the drawing process, instead let the remote "viewer"
                ...
                } else          // LCD controller/driver not busy, try to read pixels from framebuffer:
                {
                    if (0 < LCD_CopyRectFromFramebuffer_RGB(
                                ...
                                *((uint8_t*)(md380_packet + 4)), // [in] y2
                                (uint8_t*)dmesg_tx_buf +
                                    5,       // destination buffer, with rect-coords followed by 3 bytes per pixel
...
/* This copies a character string into a USB Descriptor string, which
   ...
   a byte of 0x03. */
const char* loadusbstr(char* usbstring, char* newstring, long* len) {
    int i = 0;
    *len  = 2;
...
void reboot_into_bootloader() {
     ...
     * travis says:
      ...
      Disable interrupts.
      Erase the block at 0x0800C000, so that the interrupt table is empty and the bootloader won't start the main
     application. Branch to the address stored in 0x08000004, which is the bootloader's RESET vector. The bootloader
     will then launch and wait for comms, rather than branch into the main application.
