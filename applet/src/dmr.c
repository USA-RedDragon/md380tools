...
void dump_raw_short_header(const char* tag, raw_sh_hdr_t* pkt) {
    NMPRINT("%s(sap=%d,src=%d,dst=%d,sp=%d,dp=%d) ", tag, pkt->sap, get_adr(pkt->src), get_adr(pkt->dst), pkt->sp,
            pkt->dp);
    PRINT("%s(sap=%d,src=%d,dst=%d,sp=%d,dp=%d)\n", tag, pkt->sap, get_adr(pkt->src), get_adr(pkt->dst), pkt->sp,
          pkt->dp);
}
...
inline uint8_t get_csbko(mbc_t* mbc) { return mbc->lb_pf_csbko & 0x3f; }
...
//     get_adr(data->src),get_adr(data->dst), blocks);
// }
